...
if [ -z "${AWS_PROFILE:-}" ]; then echo "ERROR aws_waf_dos_rules: AWS_PROFILE is not set" >&2; exit 1; fi
if [ -z "${AWS_DEFAULT_REGION:-}" ]; then echo "ERROR aws_waf_dos_rules: AWS_DEFAULT_REGION is not set" >&2; exit 1; fi
